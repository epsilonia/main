...
def interactive_interface():
	def get_figure(eps, N):
		...
		fig.update_layout(title=dict(text=title, font=dict(size=25)),  title_font_color=title_color)
	...
	st.markdown(r"(1) Use the first slider to change the value of $\varepsilon$")
	st.markdown(r"(2) For each chosen value of $\varepsilon$, adjust $N$ with the second slider to obtain a good value", unsafe_allow_html=True)
	st.markdown(r"A good value of $N$ is one such that all terms with indices greater than $N$ are inside the strip of center 2 and radius $\varepsilon$", unsafe_allow_html=True)
	st.markdown("""---""")
	...
	st.plotly_chart(fig, use_container_width=True, on_change=get_figure)
